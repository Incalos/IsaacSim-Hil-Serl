...
class MultiCameraBinaryRewardClassifierWrapper(gym.Wrapper):
    ...
    def compute_reward(self, obs, action):
        # Calculate action norm penalty to encourage minimal movement
        action_norm = np.linalg.norm(action[:6])
        total_reward = -0.05 * action_norm

        # Get reward and success flag from classifier function
    ...
    def step(self, action):
        # Track start time for frequency regulation
        start_time = time.time()

        # Execute original environment step
        obs, rew, done, truncated, info = self.env.step(action)

        # Calculate custom reward and update success status
        rew, success = self.compute_reward(obs, action)
        done = done or success
        info["succeed"] = success

        # Maintain target control frequency
    ...
    def reset(self, **kwargs):
        # Reset base environment and success flag
...
class MultiStageBinaryRewardClassifierWrapper(gym.Wrapper):
    ...
    def compute_reward(self, obs, action):
        # Penalize large action magnitudes to promote efficient movements
        action_norm = np.linalg.norm(action[:6])
        total_reward = -0.05 * action_norm

        # Process stages sequentially until incomplete stage found
    ...
    def step(self, action):
        ...
        # Update info dict with stage progress metrics
        info.update({
            "succeed": all_done,
    ...
    def reset(self, **kwargs):
        # Reset base environment and stage tracking
        obs, info = self.env.reset(**kwargs)
...
class GamepadIntervention(gym.Wrapper):
    ...
    def __init__(self, env: Env, guid: str):
        ...
        # Initialize movement scale factors for gamepad input
        self.scale = (0.05, 0.01, 0.09)
    ...
    def _load_urdf(self):
        # Resolve absolute path to URDF file
        urdf_path = Path(self.urdf_path)
        if not urdf_path.is_absolute():
            _root = Path(__file__).resolve().parent.parent
            urdf_path = _root / urdf_path

        # Load URDF for forward kinematics calculations
    ...
    def joints_to_eef_fk(self, joint_positions: Sequence[float]) -> np.ndarray:
        ...
        # Ensure consistent quaternion representation (positive w component)
        if quat_xyzw[3] < 0:
            quat_xyzw = -quat_xyzw
    ...
    def calculate_delta_eef(self, curr_eef: np.ndarray, next_eef: np.ndarray) -> np.ndarray:
        # Extract position components from end effector poses
        p_curr = np.array(curr_eef[:3])
        p_next = np.array(next_eef[:3])
        delta_pos = p_next - p_curr

        # Calculate rotational delta in axis-angle representation
        ...
        # Normalize deltas by environment action scale
    ...
    def process_gamepad_action(self, agent_action: np.ndarray) -> Tuple[np.ndarray, bool]:
        # Get gamepad input and intervention status
        ...
        # Parse gamepad delta values (shoulder, y, z, flex, roll, gripper)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # Record intervention action in info dict if applicable
        if intervened:
