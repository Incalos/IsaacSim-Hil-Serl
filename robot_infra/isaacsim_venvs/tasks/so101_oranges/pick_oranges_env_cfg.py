...
class PickOrangesSceneCfg(InteractiveSceneCfg):
    ...
    # Frame transformer for gripper/jaw position tracking relative to robot base
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        # State-based observations for RL policy
        ...
        def __post_init__(self) -> None:
            # Enable observation corruption for robustness training
            self.enable_corruption = True
            # Disable term concatenation for separate observation handling
            self.concatenate_terms = False

    @configclass
    class SubtaskCfg(ObsGroup):
        # Subtask status tracking for each orange (pick/place)
        ...
        def __post_init__(self):
            # Disable corruption for subtask state tracking
            self.enable_corruption = False
            # Keep subtask terms separate for individual status checking
            self.concatenate_terms = False

    # Main observation groups
...
@configclass
class EventsCfg:
    # Reset event to restore scene to default state
...
@configclass
class RewardsCfg:
    # Sparse reward for completing full task (all oranges on plate)
...
class PickOrangesEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Task metadata
    task_description: str = "Pick the orange and put it into the plate, then reset the arm to rest state."
    ...
    def __post_init__(self) -> None:
        ...
        # Frame transformer visualization scale
        self.scene.ee_frame.visualizer_cfg.markers["frame"].scale = (0.05, 0.05, 0.05)

        # Extract sub-assets from USD file (oranges + plate)
    ...
    def use_teleop_device(self, teleop_device: str) -> None:
        # Configure environment for teleoperation (keyboard/gamepad)
        self.task_type = teleop_device
        self.actions = init_action_cfg(self.actions, device=teleop_device)

        # Disable gravity for robot when using manual control
