...
class MemoryEfficientReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        # Initialize pixel keys and device configuration
        self.pixel_keys = pixel_keys
        self.device = torch.device(device)

        # Process observation space to unstack pixel dimensions
        ...
        # Create next observation space by removing pixel keys
        ...
        # Initialize index validity tracking and first insertion flag
        self._first = True
        self._is_correct_index = torch.full((capacity,), False, dtype=torch.bool, device=self.device)

        # Initialize parent ReplayBuffer class
        super().__init__(
            observation_space,
            action_space,
            capacity,
    ...
    def insert(self, data_dict: DatasetDict):
        # Handle buffer wrap-around by reinserting recent elements
        ...
        # Deep copy to avoid modifying original data and convert numpy arrays to torch tensors
        ...
        # Extract pixel data from observations and next_observations
        obs_pixels = {}
        next_obs_pixels = {}
        for pixel_key in self.pixel_keys:
        ...
        # Handle first insertion by initializing stacked pixel frames
        if self._first:
        ...
        # Update observation pixels with last frame from next observation
        for pixel_key in self.pixel_keys:
            data_dict["observations"][pixel_key] = next_obs_pixels[pixel_key][-1]

        # Update first flag based on done status and mark index as valid
        self._first = data_dict["dones"]
        self._is_correct_index[self._insert_index] = True
        super().insert(data_dict)

        # Mark subsequent indices as invalid to avoid partial stack sampling
        for i in range(self._num_stack):
    ...
    def sample(
        ...
        # Sample valid indices if not provided (skip invalid indices)
        if indx is None:
        ...
        # Prepare keys for sampling (ensure observations are included)
        if keys is None:
            keys = self.dataset_dict.keys()
        else:
            assert "observations" in keys

        # Remove observations from keys to handle custom pixel stacking
        keys = list(keys)
        keys.remove("observations")
        batch = super().sample(batch_size, keys, indx)

        # Extract non-pixel observation keys and populate batch
        ...
        # Process pixel observations with sliding window stacking
        for pixel_key in self.pixel_keys:
            ...
            # Create sliding window view for stacked frames
            obs_pixels = obs_pixels.unfold(0, self._num_stack + 1, 1)
            obs_pixels = obs_pixels[indx - self._num_stack]

            # Reorder dimensions to (batch, time, height, width, channel)
            obs_pixels = obs_pixels.permute(0, 4, 1, 2, 3)

            # Pack or split observations/next observations based on flag
