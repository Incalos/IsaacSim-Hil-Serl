...
def batched_random_crop(img: torch.Tensor, rng: torch.Generator, padding: int, num_batch_dims: int = 1) -> torch.Tensor:
    ...
    # Perform cropping and reshape to original shape
...
def resize(image: torch.Tensor, image_dim: tuple) -> torch.Tensor:
    ...
    return (F.interpolate(image.permute(2, 0, 1).unsqueeze(0), size=image_dim, mode="bilinear", align_corners=False).squeeze(0).permute(1, 2, 0))


def _maybe_apply(apply_fn, inputs: torch.Tensor, rng: torch.Generator, apply_prob: float) -> torch.Tensor:
    # Apply transform with given probability, return original if not applied
...
def hsv_to_rgb(h: torch.Tensor, s: torch.Tensor, v: torch.Tensor) -> tuple:
    # Convert HSV channels back to RGB tensors
...
def color_transform(
    ...
    # Convert image to grayscale (RGB format)
    ...
    # Check application probabilities
    ...
    if should_apply and should_apply_color:
        ...
        # Shuffle transform order if enabled
        if shuffle:
...
def gaussian_blur(
    ...
    # Calculate kernel size (odd number) based on image height
    ...
    def _apply(image):
        ...
        return (TF.gaussian_blur(image.permute(2, 0, 1).unsqueeze(0), kernel_size=[kernel_size, kernel_size], sigma=[sigma, sigma]).squeeze(0).permute(1, 2, 0))
...
def solarize(image: torch.Tensor, rng: torch.Generator, threshold: float, apply_prob: float) -> torch.Tensor:
    # Solarize function: invert pixels above threshold
    def _apply(image):
        return torch.where(image < threshold, image, 1.0 - image)

    # Apply solarization with specified probability
    return _maybe_apply(_apply, image, rng, apply_prob)
